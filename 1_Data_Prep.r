...
region_loop = tryCatch(getArgument("VAR_REGION"), error = function(e) {return('CENTRAL')}) ## ASM_CHENNAI-HOME
focus_loop = tryCatch(getArgument("VAR_FOCUS"), error = function(e) {return('HEALTH')})
country_loop = tryCatch(getArgument("VAR_COUNTRY"), error = function(e) {return('MY_HEALTH')})
region_home = tryCatch(getArgument("VAR_SCHEMA_NAME"), error = function(e) {return('ASEAN')})
country_home = tryCatch(getArgument("VAR_COUNTRY_HOME"), error = function(e) {return('MY')})
...
if (environment == 'dev'){
	if (country_loop == 'INDIA_HEALTH' | country_loop == 'PAK_HEALTH' | country_loop == 'MY_HEALTH'){
...
# MAGIC elif country_loop == 'MY_HEALTH':
# MAGIC   blob_path = "wasbs://newspageinputmy@prdrbainesasmartorderhc.blob.core.windows.net"
# MAGIC else:
# MAGIC   blob_path = "wasbs://newspageinputindiahealth@prdrbainesasmartorderhc.blob.core.windows.net"
...
# MAGIC   if country_loop == 'INDIA_HEALTH' or country_loop == 'PAK_HEALTH' or country_loop=='MY_HEALTH':
...
# MAGIC #     pattern = pattern + '20230227' ## ONLY for TESTING Purpose (2020/01/03) MS
# MAGIC   
# MAGIC   else:
...
# MAGIC #    pattern = pattern + '20230203' ## ONLY for TESTING Purpose (2020/01/03) MS
# MAGIC 
# MAGIC 
# MAGIC   print(pattern) 
...
display(NMaster_sub_unique)

# COMMAND ----------

sort(unique(NMaster_sub_unique$V1))
...
### Modified to include P5M for MY - Kapil 2/24/2023
else if (country_loop == 'MY_HEALTH'){
  {if(length(prev_dates) > 4){
  focus_dates = prev_dates[c((length(prev_dates) - 4):(length(prev_dates)))]} else {focus_dates = prev_dates}}}

# else {if(length(prev_dates) > 12){focus_dates <- prev_dates[c((length(prev_dates) - 11), (length(prev_dates) - 10),(length(prev_dates) - 3):(length(prev_dates)))]} 
# else { focus_dates <- prev_dates[c((length(prev_dates) - 3):(length(prev_dates)))]}
   
focus_dates <- format(focus_dates, "%Y/%m")
print(focus_dates)

# # valid_months <-c("2022/07", "2022/08" ,"2022/09","2022/10","2022/11","2022/12")
...
if (country_loop != 'INDIA_HOME' & country_loop != 'PAKISTAN_HOME' & country_loop != 'INDIA_HEALTH' & country_loop != 'PAK_HEALTH' & country_loop != 'MY_HEALTH') { ## Added country_loop != 'PAKISTAN_HOME' to condition, requested by Rakesh because this filter is no longer used for Pakistan too (2020/05/18) - MS
...
if ((country_loop != 'INDIA_HOME') & (country_loop != 'INDIA_HEALTH') & (country_loop != 'PAK_HEALTH') & (country_loop != 'MY_HEALTH')) { 
